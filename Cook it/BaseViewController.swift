    ...
    func slideMenuItemSelectedAtIndex(_ index: Int32) {
        ...
        if (index>=0) {
           ...
           if(index == 0){
            ...
            var aboutVC : AboutusViewController!
            aboutVC  = self.storyboard!.instantiateViewController(withIdentifier: "AboutusViewController") as! AboutusViewController
            navigationController?.pushViewController(aboutVC, animated: false)
            self.navigationController?.navigationItem.title  = "ABOUT US"
            }
           else if(index == 2) {
            var shareVC : ShareViewController!
            shareVC  = self.storyboard!.instantiateViewController(withIdentifier: "ShareViewController") as! ShareViewController
            navigationController?.pushViewController(shareVC, animated: false)
            self.navigationController?.navigationItem.title  = "SHARE"
            }
           else if(index == 3) {
            var contactVC : ContactusViewController!
            contactVC  = self.storyboard!.instantiateViewController(withIdentifier: "ContactusViewController") as! ContactusViewController
            navigationController?.pushViewController(contactVC, animated: false)
            self.navigationController?.navigationItem.title  = "CONTACT US"
            }
           else if(index == 4) {
